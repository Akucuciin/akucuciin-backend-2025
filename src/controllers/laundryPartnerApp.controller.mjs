...
const LaundryPartnerAppController = {
  toggleOpenClose: async (req, res, next) => {
    try {
      const result = await LaundryPartnerAppService.toggleOpenClose(req);
  ...
  getOrdersByLaundryPartnerId: async (req, res, next) => {
    try {
      const result =
        await LaundryPartnerAppService.getOrdersByLaundryPartnerId(req);
      return res.status(200).json({
        success: true,
        data: result,
      });
